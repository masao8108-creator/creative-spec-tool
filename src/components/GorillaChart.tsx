          ...
          {topList.map((c, i) => {
            ...
            return (
              ...
              >
                ...
                {c.profileImage ? (
                  <img src={c.profileImage} alt={c.name} className="w-11 h-11 rounded-xl object-cover" />
                ) : (
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-lg font-extrabold ${color.bg} ${color.text}`}>{c.name[0]}</div>
                )}
